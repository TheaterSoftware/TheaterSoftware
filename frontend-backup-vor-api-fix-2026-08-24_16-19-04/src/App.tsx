...
function App() {
        ...
        const assignmentsResponses =
          await Promise.all(
            loadedPerformances.map(
              async (performance) => {
                const response = await fetch(
                  `http://127.0.0.1:8000/api/assignments?performance_id=${performance.id}`
                );
  ...
  async function changePerformance(
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/assignments?performance_id=${performanceId}`
      );
  ...
  async function deletePerformance() {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/performances/${selectedPerformance.id}`,
        {
          method: "DELETE",
          headers: {
            "X-User-Id":
  ...
  async function changeStatus(
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/assignments/${assignmentId}/status`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
